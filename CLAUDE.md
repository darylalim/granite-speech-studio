...
`tests/test_app_ui.py` — end-to-end UI tests driving the real `main()` widget tree via `streamlit.testing.v1.AppTest` (complements `test_streamlit_app.py`, which mocks `st` entirely and never exercises the UI wiring). Covers default widget state, Run-button gating on audio + task presence, source-change task reset, the VAD-off long-audio warning, and an end-to-end Run that renders a result card. AppTest re-executes `streamlit_app.py` in a fresh namespace on every `.run()`, so mocks must patch the **shared upstream import** (`mlx_audio.stt.utils.load_model`) rather than `streamlit_app.load_model` — patching `streamlit_app` attributes does not cross AppTest's script-runner boundary, and clicking Run without an upstream patch would load the real ~2.9GB speech model. Pure-UI cases avoid clicking Run entirely.
